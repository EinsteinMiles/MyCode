...
from config import REST_EVERY_N, REST_MIN, REST_MAX, logger
...
class BaseScraper(ABC):
    ...
    def _safe_extract(
        self, page: ChromiumPage, selectors: List[str],
        default: str = "", attribute: str = "text",
    ) -> str:
        """多选择器降级提取"""
    ...
    def _check_captcha(self, page: ChromiumPage) -> bool:
        captcha_keywords = [
            "captcha", "robot check", "verify you are a human",
            "unusual traffic", "automated requests",
            "enter the characters", "security check",
        ]
        try:
            html = page.html[:5000].lower()
    ...
    def _new_product(
        ...
        price: float = 0.0, url: str = "", **kwargs,
    ) -> Product:
        now = now_str()
        return Product(
            platform=self.platform, product_id=product_id,
            title=title, price=price, url=url,
            first_seen=now, last_updated=now, **kwargs,
        )
