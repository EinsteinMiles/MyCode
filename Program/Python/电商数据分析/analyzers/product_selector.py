"""
选品分析器
加权综合评分模型（参考 财报分析/cleaner/analyzer.py 模式）
"""
...
class ProductSelector:
    ...
    def score_profit_potential(self, product: Product, avg_price: float) -> float:
        ...
        基于起批量价格 vs 品类均价的差额
        """
        if product.price <= 0 or product.moq <= 1:
            return 40.0

        # 起批量大 + 单价低 = 批发优势
        moq_bonus = min(20, math.log(product.moq + 1) * 3)

        if avg_price > 0 and product.price < avg_price:
            margin = (avg_price - product.price) / avg_price * 100
            return min(100, 50 + margin * 0.5 + moq_bonus)

        return 30.0 + moq_bonus
