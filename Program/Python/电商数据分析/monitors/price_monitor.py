...
class PriceMonitor:
    ...
    def check_price(self, product_db_id: int, current_price: float) -> Optional[Dict[str, Any]]:
        """
        检查价格变动
        返回告警信息 dict，无变动则返回 None
        """
        ...
        if abs(change_pct) >= PRICE_ALERT_THRESHOLD_PCT:
            ...
            alert = {
                ...
                "title": product.title if product else "未知商品",
                "old_price": old_price,
    ...
    def check_all_monitored(self, scraper_get_price=None) -> List[Dict[str, Any]]:
        """
        检查所有活跃的价格监控任务
        scraper_get_price: 可选的获取价格的函数 (product_url) -> float
        """
